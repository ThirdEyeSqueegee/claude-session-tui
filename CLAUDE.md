...
- **`orphans.go`** — the inverse of `deleteSession`: `findOrphans`/`sweepOrphans` scan the satellite dirs for state whose id has no live transcript (the live set = basenames of `projects/*/*.jsonl`) and reap it. Drives `--prune`. Dry run by default; `--apply` removes. All removals route through `confinedRemover`. Mirrors the `ccp` / `cce-find-orphans` nushell helpers. `shell-snapshots` is deliberately not swept — its filenames carry no session id.
